...
if [ "${PRODUCTION_MODE}" = "true" ]; then
    ...
    CLOUD_RUN_CONCURRENCY="${CLOUD_RUN_CONCURRENCY:-50}"
...
gcloud run deploy "${SERVICE_NAME}" \
    ...
    --set-env-vars="META_APP_SECRET=${META_APP_SECRET:-}" \
    --set-env-vars="FLOW_TOKEN_SECRET=${FLOW_TOKEN_SECRET:-}" \
    --set-env-vars="GENDEI_FUNCTIONS_URL=${GENDEI_FUNCTIONS_URL:-}" \
    --set-env-vars="GENDEI_SERVICE_SECRET=${GENDEI_SERVICE_SECRET:-}" \
    --set-env-vars="OPENAI_SESSION_BACKEND=${OPENAI_SESSION_BACKEND:-sqlalchemy}" \
    --set-env-vars="OPENAI_SESSION_DATABASE_URL=${OPENAI_SESSION_DATABASE_URL:-}" \
    --set-env-vars="OPENAI_SESSION_REDIS_URL=${OPENAI_SESSION_REDIS_URL:-}" \
    --set-env-vars="OPENAI_SESSION_HYDRATE_FROM_FIRESTORE=${OPENAI_SESSION_HYDRATE_FROM_FIRESTORE:-true}" \
    --set-env-vars="OPENAI_SESSION_BACKEND_FALLBACK_TO_SQLITE=${OPENAI_SESSION_BACKEND_FALLBACK_TO_SQLITE:-false}"
