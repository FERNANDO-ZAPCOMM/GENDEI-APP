 ...
 * Returns a unified status object for the 6-step onboarding process
 */
export function useOnboardingStatus() {
  ...
  const setupStatus = useMemo((): ClinicSetupStatus => {
    // Step 1: Clinic info is complete if clinic has a name (not default)
    ...
    // Step 4: Schedule complete if at least 1 professional has working hours configured
    const scheduleComplete = activeProfessionals.some((p: Professional) => {
      if (!p.workingHours) return false;
      // Check if at least one day has schedule configured
    ...
    // Step 5: Payment complete if payment settings are configured
    // For now, check if clinic has depositPercentage set or paymentSettings
    ...
    // Step 6: WhatsApp complete if status is CONNECTED or READY
    ...
    else if (!scheduleComplete) nextStep = 4;
    else if (!paymentComplete) nextStep = 5;
    else if (!whatsappComplete) nextStep = 6;
